...
def signuppressed():
    otp_frame = LabelFrame(
        signup_frame, font=("Arial", 15), width=261, height=138, bg="#565050", bd=2
    ).place(x=509, y=530)
    otp_frame_l = PhotoImage(file="Images/Otp Frame.png")

    otp_l = Label(
        otp_frame, text="Enter OTP here", font=("Arial", 15), fg="#C09D47", bg="#565050"
    ).place(x=576, y=540)
...
def passwcheck():
    if c_password.get() != s_password.get():
        Label(
            signup_frame, text="Password don't match", bg="#565050", font=("Arial", 10)
        ).place(x=560, y=430)

    if any(char.isdigit() for char in c_password.get()) != True:
        Label(
            signup_frame, text="Input some digit", bg="#565050", font=("Arial", 10)
        ).place(x=560, y=430)
